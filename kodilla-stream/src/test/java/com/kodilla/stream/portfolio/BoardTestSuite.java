...
public class BoardTestSuite {
    ...
    public void testAddTaskListFindOutdatedTasks() {
        ...
        List<Task> tasks = project.getTaskLists().stream()
                ...
                //.filter(list -> undoneTasks.contains(list))
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        double avgNumberOfDays = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(task -> task.getCreated())
                .mapToInt(d -> Period.between(d, now).getDays())
                .average().getAsDouble();
        ...
        for (TaskList taskList: project.getTaskLists()) {
            if(taskList.getName().equals("In progress")){
                for(Task task: taskList.getTasks()){
                    sum= sum + Period.between(task.getCreated(), now).getDays();
                    count++;
                }
            }
        }

        double expectedAvg = sum/count;

        //Then
        Assert.assertEquals(expectedAvg, avgNumberOfDays, 0);
    }
}
